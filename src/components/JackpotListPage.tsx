    ...
    <div className="space-y-8 text-left animate-fadeIn">
      ...
      <div className="relative overflow-hidden p-6 md:p-8 rounded-[var(--radius)] bg-[var(--card)] border border-[var(--border)] shadow-[var(--shadow)]">
        ...
        <div className="max-w-3xl space-y-3">
          ...
          <p className="text-xs md:text-sm text-[var(--text-muted)] leading-relaxed max-w-2xl font-medium">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 mt-6 border-t border-[var(--border)]">
          <div className="space-y-1">
            <span className="text-[10px] text-slate-700 dark:text-slate-300 uppercase font-black font-mono">Jackpot Slips</span>
            <p className="text-sm font-black text-[var(--text)]">6 Active Listings</p>
          </div>
          <div className="space-y-1">
            <span className="text-[10px] text-slate-700 dark:text-slate-300 uppercase font-black font-mono">Bonus Hit Rate</span>
            <p className="text-sm font-black text-emerald-600 dark:text-emerald-400">84.2% Verified</p>
          </div>
          <div className="space-y-1">
            <span className="text-[10px] text-slate-700 dark:text-slate-300 uppercase font-black font-mono">Payment Method</span>
            <p className="text-sm font-black text-[var(--text)]">M-Pesa Express</p>
          </div>
          <div className="space-y-1">
            <span className="text-[10px] text-slate-700 dark:text-slate-300 uppercase font-black font-mono">Unlock Window</span>
            <p className="text-sm font-black text-indigo-700 dark:text-indigo-400">Instant SMS Code</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {listData.map((jackpot) => {
            ...
            if (hasEnded) {
              statusBadge = (
                <span className="px-2 py-0.5 rounded text-[8.5px] font-black uppercase tracking-wider font-mono bg-rose-600 text-white">
                  Completed & Closed
                </span>
              );
            } else if (hasStarted) {
              statusBadge = (
                <span className="px-2 py-0.5 rounded text-[8.5px] font-black uppercase tracking-wider font-mono bg-amber-500 text-slate-950 animate-pulse">
                  Live In Progress
                </span>
              );
            } else {
            ...
            return (
              <a 
                ...
                className="group relative flex flex-col justify-between p-5 rounded-[var(--radius)] bg-[var(--card)] border border-[var(--border)] hover:border-emerald-500/40 dark:hover:border-emerald-500/30 transition-all duration-300 hover:shadow-md cursor-pointer text-left no-underline min-h-[44px]"
              >
                {/* Visual Accent */}
                <div className="absolute top-0 left-0 right-0 h-[2px] bg-slate-200 dark:bg-slate-800 group-hover:bg-emerald-500 transition-colors" />
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 min-w-0">
                      {/* Brand Badge */}
                      <span className={`px-2 py-0.5 rounded text-[9.5px] font-mono font-black uppercase tracking-wider border shrink-0 whitespace-nowrap ${brandInfo.bgClass}`}>
                        {brandInfo.brand}
                      </span>

                      {/* Country Flag & Name */}
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9.5px] font-mono font-bold bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-slate-100 border border-slate-300 dark:border-slate-700 shrink-0 whitespace-nowrap">
                      ...
                      {/* Matches Count Badge */}
                      <span className="px-2 py-0.5 rounded text-[9.5px] text-slate-900 dark:text-slate-100 bg-slate-200 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 font-mono font-bold shrink-0 whitespace-nowrap">
                    ...
                    <div className="flex items-center gap-1.5 px-3 py-1 rounded-md bg-emerald-600 dark:bg-emerald-500 text-white dark:text-slate-950 font-mono shadow-sm shrink-0 whitespace-nowrap ring-1 ring-emerald-700/20 dark:ring-emerald-400/30">
                      <span className="text-[9.5px] font-black uppercase tracking-wider opacity-95">Price:</span>
                  ...
                  <div className="space-y-1">
                    <h3 className="text-base font-black text-[var(--text)] tracking-tight group-hover:text-[var(--primary)] transition-colors uppercase">
                      {jackpot.name}
                    </h3>
                  ...
                  <div className="p-3 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800 space-y-2 text-xs">
                    <div className="flex items-center gap-2 text-slate-800 dark:text-slate-200">
                      ...
                      <span className="font-bold text-[11.5px] font-mono truncate text-slate-900 dark:text-slate-100">
                    ...
                    <div className="flex items-center justify-between text-[11.5px]">
                      ...
                      <span className="font-black font-mono text-emerald-600 dark:text-emerald-400">{jackpot.submissionsFill}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-5 pt-4 border-t border-[var(--border)] flex items-center justify-between min-h-[44px]">
                  <div className="flex items-center gap-2">
                    {isUnlocked ? (
                      ...
                      <span className="px-2 py-1 rounded text-[8.5px] font-black font-mono bg-slate-200 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-700 uppercase tracking-wider">
                        Premium
                      </span>
                    )}
                  ...
                  <div className="flex items-center gap-1.5 text-xs font-black text-[var(--primary)] group-hover:translate-x-1.5 transition-transform min-h-[44px] items-center">
