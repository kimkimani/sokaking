...
import { expandTopFixturesParameters, generateTopConfidenceFixturesMarkdown, resolveJackpotId } from '../utils/topJackpotFixtures';
...
export function parseMarkdownPage(rawMd: string, keyName: string = ''): ParsedMarkdownPage {
  ...
  const effectiveJackpotId = meta.jackpotId || resolveJackpotId(keyName || meta.pageKey, 'sportpesa-mega');
  cleanedContent = expandTopFixturesParameters(cleanedContent, effectiveJackpotId);
  ...
  if (yamlTopConf && yamlTopConf[1] !== 'false' && !cleanedContent.includes('highest confidence score')) {
    ...
    cleanedContent += `\n\n### Top SportPesa Mega Jackpot Predictions by Confidence\n\n` + generateTopConfidenceFixturesMarkdown(effectiveJackpotId, count);
  }
